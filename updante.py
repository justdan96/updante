...
"""
updante.py: Check for updates, download in background

Based on pyupdatesd by Kirsle.

Edited by Dan Bryant

pyupdatesd description:
...
c = dict(
    ...
    interval = 10800, # 10800 = 3 hours, 900 = 15 minutes
    ...
    # Command to download updates in the background
    dl = "/usr/bin/pkcon -p -d -c 1 update",
    ...
    #gui = "gpk-update-viewer",
    #gui = "yumex update",
    gui = "gnome-software --mode=updates",
)
    ...
    if int(time()) >= next_check:
        ...
        if status == 100:
            ...
            if tray.get_visible() == False:
                say("Download updates in background")
                
                status2, output2 = commands.getstatusoutput(c['dl'])
                
                say("Result status:", status2)
                say("Command output:")
                say(output2)
